...
#check for anova (independence of observations) of two columns
aov1=aov(loan_m$int.rate~loan_m$installment, data = loan_m)
summary(aov1)   #pf value 2e-16, rejects Ho, variables means are statistically different

#checking for multicollinearity
pcor(loan_m[,2:12],method = "pearson")$estimate  #seems to not have notable values of correlation between each other
...
vif_model   #fico and int.rate moderately correlated (above 2.4)
...
probs=round(logit2prob(loan_logT$coefficients), digits = 4)
...
prediclev=as.numeric(predictions>.86157)  #86157
...
library(pROC)
g <- roc(loan_m$not.fully.paid ~ predictions, data = loan_m)
plot(g)  
